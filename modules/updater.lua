...
local VERSION_TAG_PREFIX = "([-_+ ]"
local VERSION_TAG_SUFFIX = "[.]?)(%d+)$"
...
function Updater.on_selected_area(event)
    ...
    if updater.label then
        ...
        if versioning ~= 'off' then
            local tag = player.mod_settings["BlueprintExtensions_version-tag"].value
            tag = VERSION_TAG_PREFIX .. tag .. VERSION_TAG_SUFFIX
            label, found = string.gsub(label, tag, function(v, n) return v .. (n+1) end)
